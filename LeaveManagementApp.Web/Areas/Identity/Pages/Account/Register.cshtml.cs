...
namespace LeaveManagementApp.Web.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            returnUrl ??= Url.Content("~/"); // Important security feature
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");
                     await _userManager.AddToRoleAsync(user, Roles.User); // assign role "User" to this user upon registration. 
